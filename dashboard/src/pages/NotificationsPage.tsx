...
const iconMap: Record<string, { icon: typeof TrendingUp; color: string }> = {
  trade: { icon: TrendingUp, color: tokens.colors.accent },
  pnl: { icon: CheckCircle, color: tokens.colors.positive },
  alert: { icon: Bell, color: tokens.colors.purple },
  warning: { icon: AlertTriangle, color: tokens.colors.warning },
};
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: tokens.colors.textMuted }}>Notify for trades above this amount</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 13, fontFamily: tokens.fonts.mono }}>${threshold}</span>
                <input
                  type="range" min={0} max={5000} step={50} value={threshold}
                  onChange={e => setThreshold(Number(e.target.value))}
                  style={{ width: 120, accentColor: tokens.colors.accent }}
                />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: tokens.colors.textMuted }}>Daily summary and milestone notifications</div>
              </div>
              <button
                onClick={() => setPnlAlerts(!pnlAlerts)}
                style={{
                  width: 44, height: 24, borderRadius: 12, border: 'none',
                  background: pnlAlerts ? tokens.colors.accent : tokens.colors.border,
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ fontSize: 11, color: tokens.colors.textMuted }}>{n.time}</div>
                  </div>
                  <div style={{ fontSize: 12, color: tokens.colors.textSecondary, marginTop: 2 }}>{n.description}</div>
                </div>
              </div>
            </Card>
          );
