...
const volumeByChain = [
  { chain: 'Hyperliquid', volume: 90600, color: tokens.colors.purple },
  { chain: 'Ethereum', volume: 45200, color: tokens.colors.accent },
  ...
  { chain: 'Sui', volume: 3200, color: tokens.colors.positive },
];

// Fee breakdown for pie chart
const feeBreakdown = [
  { name: 'Platform Fees', value: 512.40, color: tokens.colors.accent },
  { name: 'Builder Fees (HL)', value: 45.30, color: tokens.colors.purple },
  { name: 'Copy Trade Profit Share', value: 89.20, color: tokens.colors.positive },
];
...
const statusColors: Record<string, string> = {
  running: tokens.colors.positive,
  stopped: tokens.colors.textMuted,
  error: tokens.colors.negative,
};
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: tokens.colors.textMuted, fontSize: 13, marginTop: 4 }}>CoinDCX Trading Agent — Operations Dashboard</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
        <StatCard
          ...
          changePositive
          icon={<Server size={20} color={tokens.colors.accent} />}
        />
        <StatCard
          label="Total Volume"
          ...
          changePositive
          icon={<Activity size={20} color={tokens.colors.purple} />}
        />
        <StatCard
          label="Total P&L"
          ...
          icon={<TrendingUp size={20} color={totalPnl > 0 ? tokens.colors.positive : tokens.colors.negative} />}
        />
        <StatCard
          label="Platform Fees"
          value={`$${totalFees.toFixed(2)}`}
          icon={<DollarSign size={20} color={tokens.colors.positive} />}
        />
        <StatCard
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={volumeByChain}>
              <XAxis dataKey="chain" tick={{ fontSize: 11, fill: tokens.colors.textMuted }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: tokens.colors.textMuted }} axisLine={false} tickLine={false} tickFormatter={v => `$${(v / 1000).toFixed(0)}K`} />
              <Tooltip
                contentStyle={{ background: tokens.colors.bgInput, border: `1px solid ${tokens.colors.border}`, borderRadius: 8, fontSize: 12 }}
            ...
            {feeBreakdown.map(f => (
              <div key={f.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ color: tokens.colors.textSecondary }}>{f.name}</span>
                </div>
                <span style={{ fontWeight: 600, fontFamily: tokens.fonts.mono }}>${f.value.toFixed(2)}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Code2 size={18} color={tokens.colors.purple} />
            <div style={{ fontSize: 14, fontWeight: 600 }}>Hyperliquid Builders Code</div>
            <Badge color="purple">Active</Badge>
          </div>
          <div style={{ display: 'flex', gap: 16, fontSize: 12, color: tokens.colors.textSecondary }}>
            <span>Code: <strong style={{ color: tokens.colors.text, fontFamily: tokens.fonts.mono }}>0xCoinDCXAgent</strong></span>
            <span>Fee: <strong style={{ color: tokens.colors.text }}>5 bps</strong></span>
            <span>Network: <strong style={{ color: '#f97316' }}>Mainnet</strong></span>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
          <div style={{ background: tokens.colors.bg, borderRadius: 8, padding: 14 }}>
            <div style={{ fontSize: 11, color: tokens.colors.textMuted, marginBottom: 4 }}>Total Builder Volume</div>
            <div style={{ fontSize: 20, fontWeight: 700, fontFamily: tokens.fonts.mono }}>${(90500).toLocaleString()}</div>
          </div>
          <div style={{ background: tokens.colors.bg, borderRadius: 8, padding: 14 }}>
            <div style={{ fontSize: 11, color: tokens.colors.textMuted, marginBottom: 4 }}>Builder Rebates Earned</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: tokens.colors.positive, fontFamily: tokens.fonts.mono }}>${builderFeeTotal.toFixed(2)}</div>
          </div>
          <div style={{ background: tokens.colors.bg, borderRadius: 8, padding: 14 }}>
            <div style={{ fontSize: 11, color: tokens.colors.textMuted, marginBottom: 4 }}>Perp Trades via Builder</div>
            <div style={{ fontSize: 20, fontWeight: 700, fontFamily: tokens.fonts.mono }}>{builderTrades.length}</div>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${tokens.colors.border}` }}>
              {['Trade ID', 'Asset', 'Volume', 'Fee Rate', 'Fee Earned', 'Time'].map(h => (
                <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 11, color: tokens.colors.textMuted, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {builderTrades.map(t => (
              <tr key={t.tradeId} style={{ borderBottom: `1px solid ${tokens.colors.bg}` }}>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono, fontSize: 12 }}>{t.tradeId}</td>
                <td style={{ padding: '10px 12px', fontWeight: 600 }}>{t.asset}</td>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono }}>${t.volume.toLocaleString()}</td>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono }}>{t.feeBps} bps</td>
                <td style={{ padding: '10px 12px', color: tokens.colors.positive, fontWeight: 600, fontFamily: tokens.fonts.mono }}>${t.feeUsd.toFixed(2)}</td>
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Users size={18} color={tokens.colors.accent} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {mockAgents.map(agent => (
              <tr key={agent.agentId} style={{ borderBottom: `1px solid ${tokens.colors.bg}` }}>
                ...
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono, fontSize: 12 }}>{agent.agentId}</td>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono, fontSize: 12, color: tokens.colors.textSecondary }}>{agent.userId}</td>
                <td style={{ padding: '10px 12px', fontWeight: 600 }}>{agent.strategy}</td>
                <td style={{ padding: '10px 12px' }}>
                  <Badge color="blue">{agent.chain}</Badge>
                </td>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono }}>{agent.tradesExecuted}</td>
                <td style={{ padding: '10px 12px', fontFamily: tokens.fonts.mono }}>${agent.volumeUsd.toLocaleString()}</td>
                <td style={{ padding: '10px 12px', fontWeight: 600, fontFamily: tokens.fonts.mono, color: agent.pnlUsd >= 0 ? tokens.colors.positive : tokens.colors.negative }}>
                  {agent.pnlUsd >= 0 ? '+' : ''}${agent.pnlUsd.toLocaleString()}
                </td>
                <td style={{ padding: '10px 12px', color: tokens.colors.textMuted }}>{agent.startedAt}</td>
              </tr>
            ))}
          </tbody>
        </table>
