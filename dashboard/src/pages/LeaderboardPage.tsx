            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #1e293b' }}>
                {['#', 'Trader', 'Sharpe', '30d P&L', '90d P&L', 'Win Rate', 'Max DD', 'Copiers', 'AUM', ''].map(h => (
                  <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, color: '#64748b', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {mockTraders.map((t, i) => (
                <tr
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    background: selectedTrader?.id === t.id ? '#1e293b' : undefined,
                  }}
                >
                  <td style={{ padding: '12px 16px' }}>
                    {i < 3 ? (
                      <Trophy size={16} color={i === 0 ? '#eab308' : i === 1 ? '#94a3b8' : '#cd7f32'} />
                    ) : (
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#64748b', display: 'flex', gap: 4, marginTop: 2 }}>
                      {t.chains.map(c => <Badge key={c} color="blue">{c}</Badge>)}
                    </div>
                  </td>
                  <td style={{ padding: '12px 16px', fontWeight: 700, color: '#a855f7' }}>{t.sharpe.toFixed(1)}</td>
                  <td style={{ padding: '12px 16px', color: '#22c55e', fontWeight: 600 }}>+{t.pnl30d}%</td>
                  <td style={{ padding: '12px 16px', color: '#22c55e', fontWeight: 600 }}>+{t.pnl90d}%</td>
                  <td style={{ padding: '12px 16px' }}>{t.winRate}%</td>
                  <td style={{ padding: '12px 16px', color: '#ef4444' }}>{t.maxDrawdown}%</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Users size={12} color="#64748b" /> {t.copiers.toLocaleString()}
                    </div>
                  </td>
                  <td style={{ padding: '12px 16px', color: '#64748b' }}>${(t.aum / 1000000).toFixed(1)}M</td>
                  <td style={{ padding: '12px 16px' }}>
          ...
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>Sharpe: {selectedTrader.sharpe.toFixed(1)}</div>
              </div>
              <button
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[
                { label: '30d P&L', value: `+${selectedTrader.pnl30d}%`, color: '#22c55e' },
                { label: '90d P&L', value: `+${selectedTrader.pnl90d}%`, color: '#22c55e' },
                { label: 'Win Rate', value: `${selectedTrader.winRate}%`, color: '#f1f5f9' },
                { label: 'Max DD', value: `${selectedTrader.maxDrawdown}%`, color: '#ef4444' },
              ].map(s => (
                <div key={s.label} style={{ background: '#0a0e17', borderRadius: 8, padding: 12 }}>
