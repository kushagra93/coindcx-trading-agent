...
import { MasterAgent } from '../../supervisor/supervisor.js';
...
function getMasterAgent(): MasterAgent {
  if (!masterAgent) {
    masterAgent = new MasterAgent(config.redis.url);
    masterAgent.start().catch(() => { /* handled in start */ });
  }
  return masterAgent;
}
...
export async function supervisorRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/supervisor/stats', async (request, reply) => {
    ...
    const stats = await master.getAggregateStats();
    const policy = await master.policyEngine.getPolicy();

    return {
      ...stats,
      globalHalt: await master.isHalted(),
      policy: {
  ...
  app.post('/api/v1/supervisor/agents', async (request, reply) => {
    ...
    try {
      const agent = await getMasterAgent().createAgent(body.userId, {
  ...
  app.get('/api/v1/supervisor/agents', async (request, reply) => {
    ...
    const agents = await getMasterAgent().getAllAgents(query);
  ...
  app.get('/api/v1/supervisor/agents/:agentId', async (request, reply) => {
    ...
    const agent = await getMasterAgent().getAgentDetails(agentId);
  ...
  app.post('/api/v1/supervisor/agents/:agentId/start', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().startAgent(agentId, ctx.userId);
  ...
  app.post('/api/v1/supervisor/agents/:agentId/stop', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().stopAgent(agentId, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/agents/:agentId/pause', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().pauseAgent(agentId, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/agents/:agentId/resume', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().resumeAgent(agentId, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.delete('/api/v1/supervisor/agents/:agentId', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().destroyAgent(agentId, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/agents/:agentId/force-close', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().forceClosePositions(agentId, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.put('/api/v1/supervisor/agents/:agentId/risk', async (request, reply) => {
    ...
    try {
      await getMasterAgent().overrideRiskSettings(agentId, overrides as any, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/agents/:agentId/strategy', async (request, reply) => {
    ...
    try {
      await getMasterAgent().pushStrategyUpdate(agentId, strategyUpdate, ctx.userId);
      return { success: true, agentId };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/emergency-halt', async (request, reply) => {
    ...
    await getMasterAgent().emergencyHaltAll(ctx.userId);
  ...
  app.post('/api/v1/supervisor/resume-all', async (request, reply) => {
    ...
    await getMasterAgent().resumeAll(ctx.userId);
  ...
  app.get('/api/v1/supervisor/policies', async (request, reply) => {
    ...
    const policy = await getMasterAgent().policyEngine.getPolicy();
    return { policy };
  });
  ...
  app.put('/api/v1/supervisor/policies', async (request, reply) => {
    ...
    const policy = await getMasterAgent().updateGlobalPolicies(updates as any, ctx.userId);
    return { policy };
  });
  ...
  app.get('/api/v1/supervisor/events', async (request, reply) => {
    ...
    const events = getMasterAgent().getRecentEvents(limit);
    return { events };
  });
  ...
  app.post('/api/v1/supervisor/approvals', async (request, reply) => {
    ...
    try {
      const result = await getMasterAgent().approveTradeRequest(approvalRequest);
  ...
  app.get('/api/v1/supervisor/fees', async (request, reply) => {
    ...
    try {
      const summary = await getMasterAgent().getFeeSummary(
        query.from,
        query.to,
      );
  ...
  app.post('/api/v1/supervisor/fees', async (request, reply) => {
    ...
    try {
      await getMasterAgent().recordFeeInLedger(body as any);
      return { success: true };
    } catch (err: any) {
      return reply.code(400).send({ error: err.message });
    }
  ...
  app.post('/api/v1/supervisor/agents/:agentId/hibernate', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().hibernateAgent(agentId, ctx.userId);
  ...
  app.post('/api/v1/supervisor/agents/:agentId/wake', async (request, reply) => {
    ...
    const { agentId } = request.params as { agentId: string };
    try {
      await getMasterAgent().wakeAgent(agentId, ctx.userId);
  ...
  app.get('/api/v1/supervisor/risk-snapshot', async (request, reply) => {
    ...
    try {
      const snapshot = await getMasterAgent().getGlobalRiskSnapshot();
  ...
  app.get('/api/v1/supervisor/regulatory/report', async (request, reply) => {
    ...
    try {
      const report = await getMasterAgent().generateRegulatoryReport(
        query.from,
        query.to,
      );
      return { report };
