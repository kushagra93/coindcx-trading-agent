...
export async function gatewayRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/gateway/deposit', async (request, reply) => {
    ...
    if (!body.user_id || !body.amount || !body.currency || !body.tx_id) {
      return reply.code(400).send({
        error: 'Required fields: user_id, amount, currency, tx_id',
      });
    }
  ...
  app.post('/api/v1/gateway/withdraw', async (request, reply) => {
    const ctx = getAuthContext(request);
    ...
    if (!body.amount || !body.token || !body.chain || !body.toAddress) {
      return reply.code(400).send({
        error: 'Required fields: amount, token, chain, toAddress',
      });
    }
