...
declare module '@fastify/jwt' {
  interface FastifyJWT {
    payload: { userId: string; tier?: string; agentId?: string; helperType?: string };
    user: { userId: string; tier?: string; agentId?: string; helperType?: string };
  }
}
...
export async function createServer(gateway?: WsGateway) {
  ...
  const allowedOrigins = config.nodeEnv === 'production'
    ? [
        ...
        'https://coindcx-trading-agent.pages.dev',
        'https://production.coindcx-trading-agent.pages.dev',
      ]
    : true;
  ...
  await app.register(jwt, { secret: config.gateway.jwtSecret });
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    if (request.url.startsWith('/api/v1/tokens/') || request.url.startsWith('/api/v1/chains') || request.url.startsWith('/api/v1/leaderboard') || request.url.startsWith('/api/v1/proxy/')) return;
    if (request.url.startsWith('/api/v1/chat') || request.url.startsWith('/api/v1/trade') || request.url.startsWith('/api/v1/perps')) return;
    if (request.url.startsWith('/api/v1/copy') || request.url.startsWith('/api/v1/strategies') || request.url.startsWith('/api/v1/templates')) return;
    if (request.url.startsWith('/api/v1/agent') || request.url.startsWith('/api/v1/risk') || request.url.startsWith('/api/v1/notifications')) return;
    ...
    try {
      ...
      (request as any).userId = (request as any).user.userId;
      (request as any).tier = (request as any).user.tier ?? 'user';
      return;
    } catch {
      // Fall through to adapter auth
    }
  ...
  app.setErrorHandler(async (error: any, request, reply) => {
    ...
    const msg = config.nodeEnv === 'production' ? 'Internal server error' : error.message;
    return reply.code(error.statusCode ?? 500).send({ error: msg });
  });
