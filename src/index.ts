...
import { clearKeyCache } from './wallet/key-manager.js';
...
export function registerShutdown(fn: () => Promise<void>) {
...
async function gracefulShutdown(signal: string) {
  ...
  for (const fn of shutdownHandlers) {
    try { await fn(); } catch (e) { log.error({ err: e }, 'Shutdown handler failed'); }
  }
...
async function startMasterMode() {
  ...
  const app = await startServer(gateway);

  registerShutdown(() => app.close());
  registerShutdown(() => master.stop());
  registerShutdown(() => gateway.stop());
