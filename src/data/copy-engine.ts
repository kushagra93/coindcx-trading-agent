...
async function handleSwapEvent(event: SwapEvent) {
  ...
  // Max slippage guard
  const maxSlippageBps = config.maxSlippageBps ?? 500; // 5% default
  if (event.amountSol > 0 && copyAmountUsd > 0) {
    const estimatedSlippageBps = Math.min(500, Math.round((copyAmountUsd / (event.amountUsd || 1)) * 100));
    if (estimatedSlippageBps > maxSlippageBps) {
      skipReason = `Estimated slippage (${estimatedSlippageBps}bps) exceeds max (${maxSlippageBps}bps)`;
      copyAmountUsd = 0;
    }
  }

  // MEV protection: add random 1-5s delay before copy execution
  const mevDelay = Math.floor(Math.random() * 4000) + 1000;
  await new Promise(r => setTimeout(r, mevDelay));
